...
int spng_decode_image(spng_ctx *ctx, void *out, size_t len, int fmt, int flags)
{
    ...
    if(f.apply_gamma)
    {
        ...
        if(exponent == 0.0f) return decode_err(ctx, SPNG_EGAMA);
        ...
        {
            float c = powf((float)i / max, (int)exponent) * max;
            if(c > max) c = max;
...
spng_ctx *spng_ctx_new(int flags)
{
    struct spng_alloc alloc =
    {
        ...
        .realloc_fn = heap_realloc,
        .calloc_fn = heap_calloc,
        .free_fn = heap_free
    };
...
static int file_read_fn(spng_ctx *ctx, void *user, void *data, size_t size)
{
    unused(size);
    ...
    if(fio_read(file, data, 1) != 1)
    {
        if(fio_eof(file)) return SPNG_IO_EOF;
...
static int file_write_fn(spng_ctx *ctx, void *user, void *data, size_t size)
{
    unused(size);
    ...
    if(fio_write(file, data, 1) != 1) return SPNG_IO_ERROR;
...
int spng_set_png_file(spng_ctx *ctx, fio_t *file)
{
